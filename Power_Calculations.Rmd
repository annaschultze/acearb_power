...
This uses the epi.sscc function from the epiR package, which implements power calculations described by Dupont (1988). 

These were solved to show a range of required sample sizes given a range of, expected odds ratios and prevalencee estimates. For simplicity, I assumed the exposure variables were binary in nature. (*Internal note - not sure this is technichally correct, or what the power implication of having a categorical exposure variable is*). I also decided to fix the power at 80% because this was considered the minimum required to execute the studies; this would therefore show the minimum number of cases needed given a certain exposure prevalence. 
...
The inputs that were considered valuable to vary were therefore exposure prevalence and effect size, as there was uncertainty about what was (a) a clinically plausible and relevant effect size and (b) what a plausible exposure prevalence was. The exposure prevalence should reflect the 
...
# 1.5 mentioned as the effectsize that, if this was missed, we'd miss a clinically potentially important effect. Create a dataset with only those estimates. 

out_small <- out %>% 
             filter(ODDS == 1.5) %>% 
             rowid_to_column() %>% 
             mutate(as.character(rowid)) %>% 
             mutate(row = str_c("row_",rowid)) %>% 
             select(PREV, n.total, n.case, row)

rowid_to_column(out_small)

```

I'd like to display this tibble as a nice table, so I decided to experiment with the new R package "gt", which should allow me to create this. 

I decided against showing plots, because I don't think they're as informative - we know that power increases with number of cases, what we'd like to know now I think is how many cases do we need at a minimum to be able to run this analysis which I think is easier read from a table. Perhaps I'll be able to add some nice colours to the table. 


```{r makenicetable}

out_small %>% gt(rowname_col = "row" 
                 ) 
